...
# Enhanced Contact Information Extractor with Better Email Detection
class ContactInformationExtractor:
    ...
    def __init__(self):
        ...
        self.phone_patterns = [
            ...
            r'\b(?:phone|tel|mobile|cell|contact)\s*:?\s*[\+]?[0-9\s\-\(\)\.]{7,20}\b',
...
class SemanticIntelligentRAG:
    def __init__(self, chroma_db_path: str, collection_name: str = "scraped_content"):
        ...
        # Initialize MongoDB for lead storage
        try:
    ...
    def init_mongodb_connection(self):
        ...
        # MongoDB connection string - modify as needed for your setup
        mongo_uri = os.getenv("MONGODB_URI", "mongodb://localhost:27017")
        database_name = os.getenv("MONGODB_DATABASE", "rag_chatbot")
...
async def lifespan(app: FastAPI):
    ...
    chroma_db_path = r"C:\Users\Abcom\Desktop\RAG\tech1"
    collection_name = "scraped_content"
    try:
        chatbot = SemanticIntelligentRAG(chroma_db_path, collection_name)
        print("✅ RAG Chatbot with MongoDB Contact Extraction ready!")
    except Exception as e:
        print(f"❌ Error: {str(e)}")
        chatbot = None
    yield
    ...
    # Cleanup on shutdown
    print("🛑 Shutting down...")
    if chatbot and chatbot.mongo_client:
        chatbot.close_mongodb_connection()
...
async def get_contact_info():
    if chatbot is None:
        raise HTTPException(status_code=503, detail="Chatbot not initialized")
    try:
        contact_docs = chatbot.search_for_contact_specific_content("contact information")
        contact_info = chatbot.extract_contact_from_docs(contact_docs)
        formatted_response = chatbot.contact_extractor.format_contact_response(contact_info, "contact information")
