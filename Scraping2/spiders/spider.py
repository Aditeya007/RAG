    ...
    def _extract_content_from_page(self, response):
        ...
        def mk(text: str, ctype: str):
            ...
            if len(text) >= 2:  # Very low threshold
                try:
                    item = ScrapedContentItem.from_response(response, text, content_type=ctype)
                    items.append(item)
        ...
        if title and title.strip():
            ...
            if len(clean_title) >= 3:
                try:
                    item = ScrapedContentItem.from_response(response, clean_title, content_type="title")
                    items.append(item)
                except ValueError:
                    pass
        ...
        if md and len(md.strip()) > 15:
            ...
            try:
                item = ScrapedContentItem.from_response(response, clean_meta, content_type="meta_description")
                items.append(item)
            except ValueError:
                pass
        ...
        for t in response.css("img::attr(alt), figure figcaption::text").getall():
            if t and t.strip() and len(t.strip()) > 10:
                ...
                if not self._is_boilerplate_text(clean_alt):
                    try:
                        item = ScrapedContentItem.from_response(response, clean_alt, content_type="alt_or_caption")
                        items.append(item)
                    except ValueError:
                        pass
        ...
        try:
            ...
            def yield_text(text: str, ctype: str):
                if text and isinstance(text, str) and text.strip():
                    ...
                    item = ScrapedContentItem.from_response(response, clean, content_type=ctype)
                    yield item
            ...
            for s in walk(data):
                item = ScrapedContentItem.from_response(response, s, content_type="json_text")
                yield item
                extracted_any = True
        ...
        try:
            ...
            for text in response.xpath('//text()[normalize-space() and string-length(normalize-space()) > 10]').getall():
                ...
                if 20 < len(clean) < 50000:
                    item = ScrapedContentItem.from_response(response, clean, content_type="rendered_text")
                    yield item
                    extracted_any = True
